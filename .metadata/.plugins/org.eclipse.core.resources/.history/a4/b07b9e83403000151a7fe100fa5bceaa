...
public class MainActivity  extends SimpleBaseGameActivity {
	...
	private boolean flagCardThrown0=false;
	private boolean flagCardThrown1=false;
	private boolean flagCardThrown2=false;
	private boolean flagCardThrown3=false;
	private int cardsThrownCounter=0;
	...
	private void addCard(int user, int[] singleBoundry,ArrayList<Integer> list ) {
		...
		//	Card pCard = GetCard.getcard(user*13+i+1);
			int randValue=list.get(i+user*13)+1;
			...
			@Override
		     protected void onManagedUpdate(float pSecondsElapsed)
		     {
		         ...
		         {                                      
		        	...
		        	if (this.user==0 && flagCardThrown0==false)
		        	{
		        		
		        		 x=CAMERA_WIDTH/2-35;
		        		 y=CAMERA_HEIGHT/2-75;
		        		 flagCardThrown0=true;
		        		 cardsThrownCounter++;
		        		 
		        	 }

		        	 	 if (this.user==1 && flagCardThrown1==false)
		        	 	 {
		        		 x=CAMERA_WIDTH/2+70;
		        		 y=CAMERA_HEIGHT/2-50;
		        		 
		        		 flagCardThrown1=true;
		        		 cardsThrownCounter++;
		        		  
		        	 }

		        	   if (this.user==2 && flagCardThrown2==false)
		        	   {
		        		 x=CAMERA_WIDTH/2-35;
		        		 y=CAMERA_HEIGHT/2-20;
		        		 flagCardThrown2=true;
		        		 cardsThrownCounter++;
		        		 
		        	 }

		        	  if (this.user==3 && flagCardThrown3==false)
		        	  {
		        		 x=CAMERA_WIDTH/2-140;
		        		 y=CAMERA_HEIGHT/2-50;
		        		 flagCardThrown3=true;
		        		 cardsThrownCounter++;
